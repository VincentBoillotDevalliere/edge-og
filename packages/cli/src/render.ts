...
function log(data: Record<string, unknown>) {
  try { console.log(JSON.stringify({ timestamp: new Date().toISOString(), ...data })); } catch {}
}
...
async function ensureWasmInitialized() {
  ...
  if (wasmUnavailable) throw new Error('WASM unavailable');
...
function getFontFamily(font: 'inter' | 'roboto' | 'playfair' | 'opensans', customFontUrl?: string) {
  if (customFontUrl) {
    try {
...
async function getFontsByName(fontName: 'inter' | 'roboto' | 'playfair' | 'opensans' = 'inter'): Promise<SatoriOptions['fonts']> {
  const FONT_CONFIGS = {
  ...
  const config = (FONT_CONFIGS as any)[fontName] ?? FONT_CONFIGS.inter;
  ...
  const css = await fetch(cssUrl, { headers: { 'User-Agent': 'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_8; de-at) AppleWebKit/533.21.1 (KHTML, like Gecko) Version/5.0.5 Safari/533.21.1' } });
  if (!css.ok) throw new Error(`Failed to fetch CSS for ${config.family}: ${css.status}`);
  const cssText = await css.text();
  const m = cssText.match(/src: url\(([^)]+)\) format\('(opentype|truetype)'\)/);
  if (!m || !m[1]) throw new Error(`No TTF/OTF URL in CSS for ${config.family}`);
  const fontResp = await fetch(m[1] as string);
  if (!fontResp.ok) throw new Error(`Font fetch failed ${config.family}: ${fontResp.status}`);
...
async function getFontsByUrl(fontUrl: string): Promise<SatoriOptions['fonts']> {
...
async function generateSVG(element: unknown, opts: { width?: number; height?: number; fonts?: SatoriOptions['fonts'] } = {}): Promise<string> {
  ...
  const svg = await satori(element as any, {
    ...
    loadAdditionalAsset: async (code: string, seg: string) => {
      if (code === 'emoji') {
        const hex = getEmojiHex(seg);
        if (hex) return `https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/svg/${hex}.svg`;
      }
      return '';
    },
  });
...
function DefaultTemplate({ title, description, theme, font, emoji = '🌟' }: { title?: string; description?: string; theme: 'light' | 'dark' | 'blue' | 'green' | 'purple'; font: 'inter' | 'roboto' | 'playfair' | 'opensans'; emoji?: string; }) {
...
export async function renderOpenGraphImage(params: {
  ...
  const props: { title?: string; description?: string; theme: 'light' | 'dark' | 'blue' | 'green' | 'purple'; font: 'inter' | 'roboto' | 'playfair' | 'opensans' } = { theme, font };
  if (typeof title !== 'undefined') props.title = title;
  if (typeof description !== 'undefined') props.description = description;
